...
{
    public static class FileDataPasswordHelper
    {
        #region 加密
        static RandomNumberGenerator rand = new RNGCryptoServiceProvider();

        public static ResultData MakeToPWD(string filepath, string pwd)
        {
            ...
            try
            {
                ...
                ZipOperatorHelper.ToZip(filepath, targetzip);
        ...
        private static void MakePWDFile(string targetzip, string pwd)
        {
            //获取文件信息
        ...
        private static byte[] GeneratorRandomBytes(int count)
        {
        ...
        private static SymmetricAlgorithm CreateRijndael(string passwod, byte[] iv)
        {
            byte[] tt = IV;
        ...
        /// <returns></returns>
        private static List<string> GetSEFFilePath(string targetzip)
        {
        ...
        private static byte[] GetByte64(string data)
        {
            return System.Text.Encoding.UTF8.GetBytes(data);
        }

        private static byte[] GetByte64C(Int64 data)
        {
        ...
        private static byte[] GetByte64(Int64 data)
        {
            List<byte> bytes = new List<byte>();
        ...
        private static byte[] GetHashData(byte[] bytes, byte[] key)
        {
        ...
        private static List<byte> ConvertData(int count, List<byte> bytes)
        {
            List<byte> temp = new List<byte>();
            switch (count)
            {
        ...
        private static Int64 GetMiddleData(int count)
        {
            Int64 data = 0;
            switch (count)
            {
        ...
        private static List<byte> datazipList = new List<byte>();

        public static string MakeFromPWD(string filepath, string pwd)
        {
            string zipfilepath = UnPWDFile(filepath, pwd);

            string sourcefilepath = FileIO.ZipOperatorHelper.UnZip(zipfilepath)[0];
        ...
        private static string GetTargetFilePath(string origalFilepath, string sourceFilePath)
        {
        ...
        private static string UnPWDFile(string filepath, string pwd)
        {
        ...
        private static void AnalizaData(List<byte> buffer)
        {
            int position = 9;
            int temp = position;
        ...
        private static int FindNextIndex(List<byte> buffer, int position, int nextStart, int minlength)
        {
            int index = position;
        ...
        private static int GetGroupLength(List<byte> list)
        {
            int maxlength = 0;
            if (list.Count > 2)
            {
        ...
        private static int GetLengthCount(Int64 length)
        {
            int count = 0;
        ...
        private static int GetLength(List<byte> list)
        {
            int length = 0;
            switch (list.Count)
            {
