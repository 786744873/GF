...
namespace CommonClient.MatchFile
{
    ...
    {
        public static List<BankInfo> GetLinkBankNo(string bankName)
        {
            ...
            List<Dictionary<string, string>> list = FileIO.FileRWHelper.ReadSingalxmlFile(filepath);
        ...
        public static CNAPS GetCNAPS(string filepath, string bankName, string cno)
        {
            CNAPS result = new CNAPS();
            Dictionary<string, List<Dictionary<string, string>>> list = FileIO.FileRWHelper.ReadTwicexmlFile(filepath);
        ...
        public static List<CNAP> GetDealSerialNos(string filepath, AppliableFunctionType aft)
        {
        ...
        public static List<PayerInfo> GetPayerList()
        {
        ...
        public static List<PayeeInfo> GetPayeeList(string filepath)
        {
            ...
            foreach (var item in list)
            {
                result.Add(new PayeeInfo
                {
                    ...
                    Email = item["Email"],
                    Fax = item["Fax"],
                    CertifyPaperType = DataConvertHelper.GetPayeeCertifyPaperType(item["CertifyPaperType"]),
                    ...
                    ProvinceType = DataConvertHelper.GetProvince(item["ProvinceType"])
                });
            }
            return result;
        }

        public static List<ElecTicketRelationAccount> GetElecTicketRelationAccountList()
        {
        ...
        public static List<PayeeInfo4TransferGlobal> GetPayeeTransferGlobalList()
        {
        ...
        public static List<InitiativeAllotAccount> GetInitiativeAllotAccountList()
        {
        ...
        public static Dictionary<AppliableFunctionType, List<NoticeInfo>> GetNoteList()
        {
            ...
            Dictionary<string, List<Dictionary<string, List<string>>>> list = FileIO.FileRWHelper.ReadSpecalxmlFile(filepath);
        ...
        public static Dictionary<AppliableFunctionType, CommonFieldType> GetCommonFieldList()
        {
        ...
        public static Dictionary<AppliableFunctionType, MappingsRelationSettings> GetBatchMappingSetting()
        {
            ...
            if (!File.Exists(filepath)) return dic;
            Dictionary<string, List<Dictionary<string, object>>> list = FileIO.FileRWHelper.ReadMultixmlFile(filepath);
            foreach (var kv in list)
            {
                AppliableFunctionType aft = DataConvertHelper.GetAppliableFunctionType(kv.Key);
                if (aft == AppliableFunctionType._Empty) continue;
                ...
                foreach (var item in kv.Value)
                {
                    ...
                    dic[aft].StartRowIndex = item.ContainsKey("StartRowIndex") && string.IsNullOrEmpty(item["StartRowIndex"].ToString()) ? 1 : int.Parse(((Dictionary<string, object>)item["StartRowIndex"])["#text"].ToString());
                    if (item.ContainsKey("EndRowIndex") && !string.IsNullOrEmpty(item["EndRowIndex"].ToString()))
                        dic[aft].EndRowIndex = int.Parse(((Dictionary<string, object>)item["EndRowIndex"])["#text"].ToString());
        ...
        public static void GetBaseSettings()
        {
            ...
            if (!File.Exists(filepath)) { SystemInit.InitBaseSetting(); return; }
            List<Dictionary<string, string>> list = FileIO.FileRWHelper.ReadSingalxmlFile(filepath);
            foreach (var item in list)
            {
                if (item.ContainsKey("AppliableTypes"))
                    SystemSettings.AppliableTypes = (AppliableFunctionType)long.Parse(item["AppliableTypes"]);
                else if (item.ContainsKey("AppliableTypesBar"))
                    SystemSettings.AppliableTypes4Bar = (AppliableFunctionType)long.Parse(item["AppliableTypesBar"]);
                else if (item.ContainsKey("IsMatchPassword4ShortProxyOut"))
                    SystemSettings.IsMatchPassword4ShortProxyOut = bool.Parse(item["IsMatchPassword4ShortProxyOut"]);
                else if (item.ContainsKey("ShortProxyOutPassword"))
                    SystemSettings.ShortProxyOutPassword = item["ShortProxyOutPassword"];
                else if (item.ContainsKey("IsMatchPassword4TransferOverCountry"))
                    SystemSettings.IsMatchPassword4TransferOverCountry = bool.Parse(item["IsMatchPassword4TransferOverCountry"]);
                else if (item.ContainsKey("TransferOverCountryPassword"))
                    SystemSettings.TransferOverCountryPassword = item["TransferOverCountryPassword"];
                else if (item.ContainsKey("IsMatchPassword4TransferForeignMoney"))
                    SystemSettings.IsMatchPassword4TransferForeignMoney = bool.Parse(item["IsMatchPassword4TransferForeignMoney"]);
                else if (item.ContainsKey("TransferForeignMoneyPassword"))
                    SystemSettings.TransferForeignMoneyPassword = item["TransferForeignMoneyPassword"];
                else if (item.ContainsKey("CureentLanguage"))
                    SystemSettings.CurrentLanguage = (UILang)int.Parse(item["CureentLanguage"]);
            }
        }

        public static void GetCity2ProvinceBarList()
        {
            string filepath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, @"Data\C2PBar.xml");
            if (!File.Exists(filepath)) return;

            if (SystemSettings.City2ProvinceBar == null) SystemSettings.City2ProvinceBar = new List<CNAP>();
            Dictionary<string, List<Dictionary<string, string>>> list = FileIO.FileRWHelper.ReadTwicexmlFile(filepath);
            foreach (var kv in list.Values)
            {
                foreach (var item in kv)
                    SystemSettings.City2ProvinceBar.Add(new CNAP { Code = item["CODE"], Name = item["NAME"] });
            }
        }

        private static string OringalKey(string key)
        {
            if (key.Contains("/"))
            {
                key.Replace('/', '(');
        ...
        public static List<VirtualAccountInfo> GetVirtualAllotAccountList()
        {
        ...
        public static void SaveBaseSettings()
        {
            try
            {
                ...
                node.InnerText = ((int)SystemSettings.AppliableTypes).ToString();
                pnode.AppendChild(node);
                node = doc.CreateNode(XmlNodeType.Element, "AppliableTypesBar", "");
                node.InnerText = ((int)SystemSettings.AppliableTypes4Bar).ToString();
                pnode.AppendChild(node);
                node = doc.CreateNode(XmlNodeType.Element, "IsMatchPassword4ShortProxyOut", "");
                node.InnerText = SystemSettings.IsMatchPassword4ShortProxyOut.ToString();
                pnode.AppendChild(node);
                node = doc.CreateNode(XmlNodeType.Element, "ShortProxyOutPassword", "");
                node.InnerText = SystemSettings.ShortProxyOutPassword;
                pnode.AppendChild(node);
                node = doc.CreateNode(XmlNodeType.Element, "IsMatchPassword4TransferOverCountry", "");
                node.InnerText = SystemSettings.IsMatchPassword4TransferOverCountry.ToString();
                pnode.AppendChild(node);
                node = doc.CreateNode(XmlNodeType.Element, "TransferOverCountryPassword", "");
                node.InnerText = SystemSettings.TransferOverCountryPassword;
                pnode.AppendChild(node);
                node = doc.CreateNode(XmlNodeType.Element, "IsMatchPassword4TransferForeignMoney", "");
                node.InnerText = SystemSettings.IsMatchPassword4TransferForeignMoney.ToString();
                pnode.AppendChild(node);
                node = doc.CreateNode(XmlNodeType.Element, "TransferForeignMoneyPassword", "");
                node.InnerText = SystemSettings.TransferForeignMoneyPassword;
                pnode.AppendChild(node);
                node = doc.CreateNode(XmlNodeType.Element, "CureentLanguage", "");
                node.InnerText = ((int)SystemSettings.CurrentLanguage).ToString();
                pnode.AppendChild(node);
        ...
        public static void SaveBatchChangeSetting(Dictionary<AppliableFunctionType, MappingsRelationSettings> dic)
        {
            try
            {
                string filepath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, @"Data\MappingsRelation.xml");
                ...
                foreach (var kv in dic)
                {
                    ...
                    n = doc.CreateNode(XmlNodeType.Element, "StartRowIndex", "");
                    n.InnerText = kv.Value.StartRowIndex.ToString();
                    node.AppendChild(n);

                    n = doc.CreateNode(XmlNodeType.Element, "EndRowIndex", "");
                    n.InnerText = kv.Value.EndRowIndex != null ? kv.Value.EndRowIndex.ToString() : string.Empty;
                    node.AppendChild(n);
        ...
        public static void SaveCommonFieldList(Dictionary<AppliableFunctionType, CommonFieldType> dic)
        {
            try
            {
                string filepath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, @"Data\CommonField.xml");
        ...
        public static void SaveNoteList(Dictionary<AppliableFunctionType, List<NoticeInfo>> dic)
        {
            try
            {
                string filepath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, @"Data\NoticeInfo.xml");
        ...
        public static void SavePayeeList(List<PayeeInfo> list, string filepath)
        {
            try
            {
                //string filepath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, @"Data\PayeeInfo.xml");
                ...
                foreach (var item in list)
                {
                    ...
                    n = doc.CreateNode(XmlNodeType.Element, "ProvinceType", "");
                    n.InnerText = ((Int32)item.ProvinceType).ToString();
                    node.AppendChild(n);
        ...
        public static void SaveElecTicketRelationAccountList(List<ElecTicketRelationAccount> list)
        {
            try
            {
                string filepath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, @"Data\ElecTicketRelationAccount.xml");
        ...
        public static void SaveInitiativeAllotAccountList(List<InitiativeAllotAccount> list)
        {
            try
            {
                string filepath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, @"Data\InitiativeAllotAccount.xml");
        ...
        public static void SavePayeeTransferGlobalList(List<PayeeInfo4TransferGlobal> list)
        {
            try
            {
                string filepath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, @"Data\PayeeTransferGlobalInfo.xml");
        ...
        public static void SavePayerList(List<PayerInfo> list)
        {
            try
            {
                string filepath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, @"Data\PayerInfo.xml");
        ...
        public static void SaveVirtualAllotAccountList(List<VirtualAccountInfo> list)
        {
            try
            {
                string filepath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, @"Data\VirtualAllotAccount.xml");
        ...
        public static void SaveAllMappingSettings()
        {
            try
            {
                ...
                foreach (var item in PrequisiteDataProvideNode.InitialProvide.AppliableFunctionTypeList)
                {
                    ...
                    MappingsRelationSettings mrs = SystemInit.GetMappingRelation(item);
                    List<string> list = SystemInit.GetRegexDescription(item);
                    int index = 0;
                ...
                FileIO.FileRWHelper.ExportTXTDocument(content, filepath);
            }
            catch { }
        }
    }
