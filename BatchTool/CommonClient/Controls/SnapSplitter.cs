                ...
                case DockStyle.Right:
                    {
                        ...
                        var handleRect = new Rectangle(_rr.X, _rr.Y, this.ClientRectangle.Width, _ssLeft.Height);

                        if (this._currentState == SplitterState.Collapsed)
                            g.DrawImageUnscaled(_ssRight, handleRect);
                        else if (this._currentState == SplitterState.Expanded)
                            g.DrawImageUnscaled(_ssLeft, handleRect);
                ...
                case DockStyle.Left:
                    {
                        ...
                        if (this._currentState == SplitterState.Collapsed)
                            g.DrawImageUnscaled(_ssLeft, _rr);
                        else if (this._currentState == SplitterState.Expanded)
                            g.DrawImageUnscaled(_ssRight, _rr);

                        if (_hot)
                            g.FillRectangle(new SolidBrush(_hotColor), _rr);
                    }
                    break;
                case DockStyle.Bottom:
                    {
                        _rr = new Rectangle(r.X + ((r.Width - _ssUp.Width) / 2), r.Y, _ssUp.Width, _ssUp.Height);
                        //var handleRect = new Rectangle(_rr.X, _rr.Y, _ssUp.Width, this.ClientRectangle.Height);

                        if (this._currentState == SplitterState.Collapsed)
                            g.DrawImageUnscaled(_ssUp, _rr);
                        else if (this._currentState == SplitterState.Expanded)
                            g.DrawImageUnscaled(_ssDown, _rr);
