...
namespace CommonClient.VisualParts2
{
    public partial class UnitivePaymentFCPayerSelector : BaseUc
    {
        ...
        void cmbCashType_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            CommandCenter.ResolveCashTypeChanged(OperatorCommandType.CardTypeChanged, (cmbCashType.Tag as List<CashType>)[cmbCashType.SelectedIndex], AppliableFunctionType.UnitivePaymentFC, false);
        }
        ...
        void CommandCenter_OnOverCountryPayeeAccountTypeEventHandler(object sender, OverCountryPayeeAccountTypeEventArgs e)
        {
            ...
            else
            {
                ...
                CommandCenter.ResolveOverCountryPayeeAccountTypeChanged(OperatorCommandType.PayeeAccountTypeChanged, e.AppType, e.AccountType, false);
            }
            SetRegex();
        }
        ...
        {
            tbRealPayerAccount.DvRegCode = "reg57";
            tbRealPayerAccount.DvMinLength = 12;
            tbRealPayerAccount.DvMaxLength = 18;
            tbRealPayerAccount.DvRequired = true;
            if (rbOut.Checked)
            {
                tbRealPayerName.DvRegCode = "reg661";
                tbRealPayerName.DvMinLength = 0;
                tbRealPayerName.DvMaxLength = 140;
                tbRealPayerName.DvRequired = false;
                tbNominalPayerAccount.DvRegCode = "reg702";
                tbNominalPayerAccount.DvMinLength = 1;
                tbNominalPayerAccount.DvMaxLength = 35;
                tbNominalPayerAccount.DvRequired = true;
                tbNominalPayerName.DvRegCode = "reg702";
                tbNominalPayerName.DvMinLength = 1;
                tbNominalPayerName.DvMaxLength = 140;
                tbNominalPayerName.DvRequired = true;
            }
            else
            {
                tbRealPayerName.DvRegCode = "reg702";
                tbRealPayerName.DvMinLength = 0;
                tbRealPayerName.DvMaxLength = 140;
                tbRealPayerName.DvRequired = false;
                tbNominalPayerAccount.DvRegCode = "reg766";
                tbNominalPayerAccount.DvMinLength = 1;
                tbNominalPayerAccount.DvMaxLength = 35;
                tbNominalPayerAccount.DvRequired = true;
                tbNominalPayerName.DvRegCode = "reg661";
                tbNominalPayerName.DvMinLength = 1;
                tbNominalPayerName.DvMaxLength = 200;
                tbNominalPayerName.DvRequired = true;
            }
        }
        ...
        public bool CheckData()
        {
            ...
            //rd = DataCheckCenter.CheckPayerAccountUP(tbRealPayerAccount, tbRealPayerAccount.Text.Trim(), lbRealPayerAccount.Text.Substring(0, lbRealPayerAccount.Text.Length - 1), 12, 18, this.errorProvider1);
            //if (!rd.Result) return rd.Result;
            //if (rbOut.Checked)
            //{
            //    if (!string.IsNullOrEmpty(tbRealPayerName.Text.Trim()))
            //    {
            //        rd = DataCheckCenter.CheckPayerNameOrBankNameUP(tbRealPayerName, tbRealPayerName.Text.Trim(), lbRealPayerName.Text.Substring(0, lbRealPayerName.Text.Length - 1), 140, this.errorProvider1);
            //        if (!rd.Result) return rd.Result;
            //    }
            //    rd = DataCheckCenter.CheckPayeeNameOrAddressGJEx(tbNominalPayerAccount, tbNominalPayerAccount.Text.Trim(), LblNominalPayerAccount.Text.Substring(0, LblNominalPayerAccount.Text.Length - 1), 35, this.errorProvider1);
            //    if (!rd.Result) return rd.Result;
            //    rd = DataCheckCenter.CheckPayeeNameOrAddressGJEx(tbNominalPayerName, tbNominalPayerName.Text.Trim(), LblNominalPayerName.Text.Substring(0, LblNominalPayerName.Text.Length - 1), 140, this.errorProvider1);
            //    if (!rd.Result) return rd.Result;
            //}
            //else
            //{
            //    if (!string.IsNullOrEmpty(tbRealPayerName.Text.Trim()))
            //    {
            //        rd = DataCheckCenter.CheckPayerNameOrBankNameUP(tbRealPayerName, tbRealPayerName.Text.Trim(), lbRealPayerName.Text.Substring(0, lbRealPayerName.Text.Length - 1), 200, this.errorProvider1);
            //        if (!rd.Result) return rd.Result;
            //    }
            //    rd = DataCheckCenter.CheckAccountUP(tbNominalPayerAccount, tbNominalPayerAccount.Text.Trim(), LblNominalPayerAccount.Text.Substring(0, LblNominalPayerAccount.Text.Length - 1), 35, this.errorProvider1);
            //    if (!rd.Result) return rd.Result;
            //    rd = DataCheckCenter.CheckPayerNameOrBankNameUP(tbNominalPayerName, tbNominalPayerName.Text.Trim(), LblNominalPayerName.Text.Substring(0, LblNominalPayerName.Text.Length - 1), 200, this.errorProvider1);
            //    if (!rd.Result) return rd.Result;
            //}
            //if (cmbCashType.SelectedIndex < 0)
            //{ this.errorProvider1.SetError(cmbCashType, string.Format("{0} {1}", MultiLanguageConvertHelper.DesignMain_Please_Selection, lbCashType.Text.Substring(0, lbCashType.Text.Length - 1))); return false; }
            return rd.Result;
        }
        ...
        {
            if (!m_isCallBack)
                CommandCenter.ResolveOverCountryPayeeAccountTypeChanged(OperatorCommandType.PayeeAccountTypeRequest, AppliableFunctionType.UnitivePaymentFC, rbOut.Checked ? OverCountryPayeeAccountType.ForeignAccount : OverCountryPayeeAccountType.BocAccount | OverCountryPayeeAccountType.OtherAccount, false);
        }
    }
}
