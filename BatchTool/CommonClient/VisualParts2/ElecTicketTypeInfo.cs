...
namespace CommonClient.VisualParts2
{
    public partial class ElecTicketTypeInfo : BaseUc
    {
        ...
        void cmbElecTicketType_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            CommandCenter.ResolveElecTicketTypeChanged(OperatorCommandType.TicketTypeChanged, m_appType, ett, RelatePersonType.Exchange);
        }
        ...
        {
            //if (cmbElecTicketType.SelectedIndex < 0) { MessageBoxPrime.Show("请选择票据类型", CommonInformations.GetFormMainTextByVersion(), MessageBoxButtonsEx.OK, MessageBoxIcon.Warning); return; }
            //校验金额
            //tbRemitAmount.DvRegCode = "reg43";
            //tbRemitAmount.DvMinLength = 1;
            //tbRemitAmount.DvMaxLength = 18;
        }
        ...
        private void FormatCash()
        {
            if (!string.IsNullOrEmpty(tbRemitAmount.Text))
            {
                ...
                rd = DataCheckCenter.CheckCash(tbRemitAmount, tbRemitAmount.Text.Trim(), tbRemitAmount.Text.Substring(0, tbRemitAmount.Text.Length - 1), 18, false, this.errorProvider1);
                if (!rd.Result) return;
                tbRemitAmount.Text = DataConvertHelper.FormatCash(tbRemitAmount.Text.Trim(), false);
            }
        }
        ...
        public bool CheckData()
        {
            ...
            //if (cmbElecTicketType.SelectedIndex < 0) { MessageBoxPrime.Show("请选择票据类型", CommonInformations.GetFormMainTextByVersion(), MessageBoxButtonsEx.OK, MessageBoxIcon.Warning); return false; }
            //rd = DataCheckCenter.CheckCash(tbRemitAmount, tbRemitAmount.Text.Trim(), lbRemitAmount.Text.Substring(0, lbRemitAmount.Text.Length - 1), 18, false, this.errorProvider1);
            //if (!rd.Result) return rd.Result;
            rd.Result = rd.Result && DataCheckCenter.CheckCash(tbRemitAmount, tbRemitAmount.Text.Trim(), lbRemitAmount.Text.Substring(0, lbRemitAmount.Text.Length - 1), 18, false, this.errorProvider1).Result;
            return rd.Result;
        }

        private void tbRemitAmount_TextChanged(object sender, EventArgs e)
        {
            lbAmountDesc.Text = DataConvertHelper.ConvertA2CN(tbRemitAmount.Text.Trim(), 18);
        }
    }
}
