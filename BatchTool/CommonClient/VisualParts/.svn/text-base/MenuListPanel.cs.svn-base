    ...
    public partial class MenuListPanel : BaseUc
    {
        ...
        void CommandCenter_OnMoveMenuEventHandler(object sender, MoveMenuEventArgs e)
        {
            ...
            else
            {
                if (e.Command == OperatorCommandType.MoveMenuRequest)
                {
                    ...
                    CommandCenter.ResolveMoveMenu(OperatorCommandType.MoveMenuCallBack, showIndex, showCount, 0, startposition, endposition, OperatorCommandType.MoveMenuCallBack);
                }
        ...
        private void ReloadMenuList()
        {
            ...
            foreach (var afType in PrequisiteDataProvideNode.InitialProvide.AppliableFunctionTypeList)
            {
                ...
                if ((int)afType > 0)
                    ShowControl(afType, (SystemSettings.AppliableTypes & afType) == afType);
                else if ((SystemSettings.CurrentVersion & VersionType.v403bar) == VersionType.v403bar)
                    ShowControl(afType, (SystemSettings.AppliableTypes4Bar & (AppliableFunctionType)Math.Abs((long)afType)) == (AppliableFunctionType)Math.Abs((long)afType));
            }
        ...
        private void btn_Click(object sender, EventArgs e)
        {
            ...
            (sender as ThemedButton).ThemeName = ThemeName.Corp_Red;
            if ((sender as ThemedButton).Tag == null) return;
            List<object> list = (sender as ThemedButton).Tag as List<object>;
            CommandCenter.ResolveShowPanel(OperatorCommandType.View, (AppliableFunctionType)list[0], (FunctionInSettingType)list[1]);
        }
    }
}
