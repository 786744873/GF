    ...
    public partial class ElecTicketOtherPanel : BaseUc
    {
        ...
        public bool CheckData()
        {
                ...
                || m_appType == AppliableFunctionType.ElecTicketTipExchange)
            {
                ...
                {
                    rd = DataCheckCenter.Instance.CheckBillSerialNo(tbBillSerialNo, tbBillSerialNo.Text.Trim(), lbBillSerialNo.Text.Substring(0, lbBillSerialNo.Text.Length - 1), 60, this.errorProvider1);
                    if (!rd.Result) return rd.Result;
                }
                if (!string.IsNullOrEmpty(tbContractNo.Text.Trim()))
                {
                    rd = DataCheckCenter.Instance.CheckContractNo(tbContractNo, tbContractNo.Text.Trim(), lbContractNo.Text.Substring(0, lbContractNo.Text.Length - 1), 60, this.errorProvider1);
                    if (!rd.Result) return rd.Result;
                }
            }
            if (!string.IsNullOrEmpty(tbNote.Text.Trim()))
            {
                rd = DataCheckCenter.Instance.CheckAddtion(tbNote, tbNote.Text.Trim(), lbNote.Text.Substring(0, lbNote.Text.Length - 1), 512, this.errorProvider1);
                if (!rd.Result) return rd.Result;
            }
            return rd.Result;
        }
