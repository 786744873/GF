...
namespace BOC_BATCH_TOOL.VisualParts
{
    public partial class VirtualAllotAccountEditor : BaseUc
    {
        ...
        bool CheckData()
        {
            ...
            rd = DataCheckCenter.Instance.CheckElecTicketPersonAccountFC(edtAccount, edtAccount.Text.Trim(), lblAccount.Text.Substring(0, lblAccount.Text.Length - 1), 35, this.errorProvider1);
            if (!rd.Result) return rd.Result;

            rd = DataCheckCenter.Instance.CheckVirtualAccountName(edtName, edtName.Text.Trim(), lblName.Text.Substring(0, lblName.Text.Length - 1), 120, this.errorProvider1);
            if (!rd.Result) return rd.Result;
