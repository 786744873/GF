...
namespace BOC_BATCH_TOOL.VisualParts
{
    public partial class BatchMappingItemsPanel : BaseUc
    {
            ...
            else
            {
                try
                {
                    if (OperatorCommandType.MappingSettingRequest == e.Command)
                    {
                        ...
                        {
                            GetItem();
                            CommandCenter.Instance.ResolveFieldMapping(OperatorCommandType.MappingSettingResolve, m_AppType, m_Mappings, m_FunType, FunctionInSettingType.MapSetting);
                    ...
                    {
                        MappingsRelationSettings mrs = SystemSettings.Instance.BatchMappingSettings.ContainsKey(e.AppType) ? SystemSettings.Instance.BatchMappingSettings[e.AppType] : null;
                        if (e.AppType == AppliableFunctionType._Empty)
                            mrs = SystemSettings.Instance.BatchSettingsMappingSettings.ContainsKey(e.BatchAppType) ? SystemSettings.Instance.BatchSettingsMappingSettings[e.BatchAppType] : null;
                        ...
                        {
                            if (e.AppType != AppliableFunctionType._Empty)
                                str = BOC_BATCH_TOOL.Utilities.EnumNameHelper<AppliableFunctionType>.GetEnumDescription(m_AppType);
                            else
                                str = BOC_BATCH_TOOL.Utilities.EnumNameHelper<FunctionInSettingType>.GetEnumDescription(m_FunType);
                        }
        ...
        {
            ResultData rd = DataCheckCenter.Instance.CheckMaxCountPreOperator(tbMaxCount, tbMaxCount.Text.Trim(), this.errorProvider1);
            if (!rd.Result) return rd.Result;
            rd = DataCheckCenter.Instance.CheckStartRowIndex(tbStartRowIndex, tbStartRowIndex.Text.Trim(), label5.Text.Substring(0, label5.Text.Length - 1), 5, this.errorProvider1);
            if (!rd.Result) return rd.Result;
            if (!string.IsNullOrEmpty(tbSeparator.Text.Trim()))
            {
                rd = DataCheckCenter.Instance.CheckSeparator(tbSeparator, tbSeparator.Text, this.errorProvider1);
                if (!rd.Result) return rd.Result;
            }
            return true;
        }
        ...
        private void SetDesc()
        {
            ...
            {
                MappingsRelationSettings mrs = SystemInit.Instance.GetMappingRelation(m_AppType);
                ...
                List<string> desList = SystemInit.Instance.GetRegexDescription(m_AppType); //SystemSettings.Instance.RegexDescriptions[m_AppType];
            ...
            {
                MappingsRelationSettings mrs = SystemInit.Instance.GetMappingRelation(m_FunType);
                string[] fieldList = new string[mrs.FieldsMappings.Count];
                mrs.FieldsMappings.Keys.CopyTo(fieldList, 0);
                if (fieldList.Length == dgv.Rows.Count)
                {
                ...
                List<string> desList = SystemInit.Instance.GetRegexDescription(m_FunType); //SystemSettings.Instance.RegexDescriptions[m_AppType];
                if (desList.Count == dgv.Rows.Count)
                {
                    for (int i = 0; i < dgv.Rows.Count; i++)
                    {
        ...
        private void llTemplate_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBoxExHelper.Instance.Show(MultiLanguageConvertHelper.Instance.Settings_Mappings_Select_Business_Type, CommonInformations.Instance.GetFormMainTextByVersion(), MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
            ...
            {
                MessageBoxExHelper.Instance.Show(MultiLanguageConvertHelper.Instance.DesignMain_Select_File_Agent, CommonInformations.Instance.GetFormMainTextByVersion(), MessageBoxButtons.OK, MessageBoxIcon.Warning); return;
            }
            ...
            if (filePath.EndsWith(".txt") || filePath.EndsWith(".csv"))
            {
                if (tbSeparator.Text.Length == 0)
                {
                    ...
                    MessageBoxExHelper.Instance.Show(MultiLanguageConvertHelper.Instance.Settings_Mappings_MakeSure_SeparaterChar_First, CommonInformations.Instance.GetFormMainTextByVersion(), MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;
                }
            }
            int rowindex = 1;
            ...
            List<string> list = MatchFile.MatchFileDataHelper.Instance.GetFileHeader(filePath, tbSeparator.Text.Trim(), rowindex);
            lboxFields.Items.Clear();

            if (list.Count == 0) { MessageBoxExHelper.Instance.Show(MultiLanguageConvertHelper.Instance.Settings_Mappings_No_Table_Header, CommonInformations.Instance.GetFormMainTextByVersion(), MessageBoxButtons.OK, MessageBoxIcon.Warning); return; }
        ...
        private void rb_CheckedChanged(object sender, EventArgs e)
        {
            ...
            if (rbBatch.Checked)
            {
                ...
                List<string> desList = SystemSettings.Instance.BatchRegexDescriptions[m_AppType];
                if (desList.Count == dgv.Rows.Count)
                {
                    for (int i = 0; i < dgv.Rows.Count; i++)
                    {
            ...
            else if (rbSingal.Checked)
            {
                ...
                List<string> desList = SystemSettings.Instance.RegexDescriptions[m_AppType];
                if (desList.Count == dgv.Rows.Count)
                {
                    for (int i = 0; i < dgv.Rows.Count; i++)
                    {
        ...
        private void PreFileData(string filepath)
        {
            ...
            if (filepath.ToLower().EndsWith(".txt"))
                filecontent = BOC_BATCH_TOOL.FileIO.FileRWHelper.Instance.ReadTXTDocument(filepath);
            else if (filepath.ToLower().EndsWith(".csv"))
                filecontent = BOC_BATCH_TOOL.FileIO.FileRWHelper.Instance.ReadCSVFieldsData(filepath);
            else filecontent = BOC_BATCH_TOOL.FileIO.FileRWHelper.Instance.ReadExcelFieldsData(filepath, 3, 1);
            ...
            else
            {
                ...
                if (filepath.ToLower().EndsWith(".txt") || filepath.ToLower().EndsWith(".csv"))
                {
                    temp = temp.Replace(tbSeparator.Text, SysCoach.SystemSettings.Instance.BOCSeparator);
                }
