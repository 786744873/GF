...
namespace BOC_BATCH_TOOL.VisualParts
{
    public partial class PayerItemsPanel : BaseUc
    {
        public PayerItemsPanel()
        {
            ...
            this.Load += new EventHandler(PayerItemsPanel_Load);
            ...
            if (SystemSettings.Instance.PayerList.Count > 0)
            {
                List<PayerInfo> list = new List<PayerInfo>();
                list.AddRange(SystemSettings.Instance.PayerList.GetRange(0, SystemSettings.Instance.PayerList.Count));
                dgv.DataSource = list;
                dgv.Refresh();
            }
        ...
        void CommandCenter_OnSettingsOperateEventHandler(object sender, SettingsOperateEventArgs e)
        {
            ...
            else
            {
                ...
                else if (e.Command == OperatorCommandType.Delete)
                {
                    ...
                    {
                        CommandCenter.Instance.ResolvePayer(OperatorCommandType.Delete, list[item], AppliableFunctionType._Empty, item);
                    }
                }
            }
        }
        ...
        void PayerItemsPanel_Load(object sender, EventArgs e)
        {
            isLoad = true;
            ...
            else
            {
                ...
                if (OperatorCommandType.Submit == e.Command)
                {
                    ...
                    if (e.OwnerType != AppliableFunctionType._Empty)
                    {
                        ...
                        else if (e.PayerInfo.ServiceListBar == AppliableFunctionType._Empty && e.OwnerType < 0) e.PayerInfo.ServiceListBar = (AppliableFunctionType)Math.Abs((int)e.OwnerType);
                    ...
                    else
                    {
                        ...
                        {
                            if (e.OwnerType == AppliableFunctionType._Empty)
                                MessageBoxExHelper.Instance.Show(MultiLanguageConvertHelper.Instance.Settings_PayerMsg_Payer_Exist_Add_Fail, CommonInformations.Instance.GetFormMainTextByVersion(), MessageBoxButtons.OK, MessageBoxIcon.Error);
                            return;
                        }
                ...
                else if (OperatorCommandType.Edit == e.Command)
                {
                    ...
                    {
                        if (e.OwnerType == AppliableFunctionType._Empty)
                            MessageBoxExHelper.Instance.Show(MultiLanguageConvertHelper.Instance.Settings_PayerMsg_Payer_Exist_Update_Fail, CommonInformations.Instance.GetFormMainTextByVersion(), MessageBoxButtons.OK, MessageBoxIcon.Error);
                        return;
                    }
        ...
        private void dgv_MouseDoubleClick(object sender, MouseEventArgs e)
        {
            ...
            CommandCenter.Instance.ResolvePayer(OperatorCommandType.View, payer, AppliableFunctionType._Empty, hitInfo.RowIndex + 1);
        }
        ...
        private void dgv_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            CommandCenter.Instance.ResolvePayer(OperatorCommandType.Delete, payer, AppliableFunctionType._Empty, e.RowIndex);
        }
    }
}
