...
namespace BOC_BATCH_TOOL.VisualParts
{
    public partial class SettingsPanel : BaseUc
    {
        public SettingsPanel()
        {
            ...
            LoadBatchChangeType(SystemSettings.Instance.AppliableTypes, SystemSettings.Instance.AppliableTypes4Bar);
        ...
        void SettingsPanel_Load(object sender, EventArgs e)
        {
            ...
            listR.AddRange(SystemSettings.Instance.PayerList);
            List<PayeeInfo> listE = new List<PayeeInfo>();
            listE.AddRange(SystemSettings.Instance.PayeeList);
            List<ElecTicketRelationAccount> listER = new List<ElecTicketRelationAccount>();
            listER.AddRange(SystemSettings.Instance.ElecTicketRelationAccountList);
            CommandCenter.Instance.ResolveAppliableFunction(OperatorCommandType.AppVisiableResolve, SystemSettings.Instance.AppliableTypes, SystemSettings.Instance.AppliableTypes4Bar);
            CommandCenter.Instance.ResolvePayer(OperatorCommandType.Load, listR, AppliableFunctionType._Empty);
            CommandCenter.Instance.ResolvePayee(OperatorCommandType.Load, listE, AppliableFunctionType._Empty);
            CommandCenter.Instance.ResolveElecTicketRelateAccount(OperatorCommandType.Load, listER);
            CommandCenter.Instance.ResolveMoveMenu(OperatorCommandType.View, 0, 0, 0, 0, 0, OperatorCommandType.View);
        ...
        {
            CommandCenter.Instance.ResolveAppliableFunction(OperatorCommandType.AppVisiableRequest, AppliableFunctionType._Empty, AppliableFunctionType._Empty);
            ...
            CommandCenter.Instance.ResolveShowPanel(OperatorCommandType.View, AppliableFunctionType._Empty, FunctionInSettingType.FunctionSetting);
        }
        ...
        private void btnMappingSave_Click(object sender, EventArgs e)
        {
            ...
            CommandCenter.Instance.ResolveFieldMapping(OperatorCommandType.MappingSettingRequest, aft, null, fst, FunctionInSettingType.MapSetting);
        }

        private void btnSetFixedInfo_Click(object sender, EventArgs e)
        {
            CommandCenter.Instance.ResolveCommonData(OperatorCommandType.CommonFieldRequest, null);
        }
        ...
        private void cmbFunctionTypeForBatchMap_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            CommandCenter.Instance.ResolveFieldMapping(OperatorCommandType.View, aft, null, fst, FunctionInSettingType.MapSetting);
        }
        ...
        private void cmbFunctionTypeForComments_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            CommandCenter.Instance.ResolveNotice(OperatorCommandType.View, SystemSettings.Instance.Notices[aft], aft);
        }
        ...
        {
            if (null == ((BOC_BATCH_TOOL.Controls.ThemedButton)sender).Tag) return;
            FunctionInSettingType fst = (FunctionInSettingType)((BOC_BATCH_TOOL.Controls.ThemedButton)sender).Tag;
            OpenFileDialog ofDialog = new OpenFileDialog();
            ofDialog.Filter = string.Format("{0}|*.txt", MultiLanguageConvertHelper.Instance.DesignMain_FileType_TXT);
            if (fst == FunctionInSettingType.ElecTicketRelateAccountMg) ofDialog.Filter += string.Format("|{0}|*.csv", MultiLanguageConvertHelper.Instance.DesignMain_FileType_CSV);
            if (ofDialog.ShowDialog() == System.Windows.Forms.DialogResult.OK)
            {
                string endstr = ofDialog.FilterIndex == 1 ? ".txt" : ".csv";
                string filepath = ofDialog.FileName;
                if (!filepath.ToLower().EndsWith(endstr)) { MessageBoxExHelper.Instance.Show(MultiLanguageConvertHelper.Instance.DesignMain_Select_File_Agent, CommonInformations.Instance.GetFormMainTextByVersion(), MessageBoxButtons.OK, MessageBoxIcon.Warning); return; }
                else filepath = DataConvertHelper.Instance.FormatFileName(filepath, endstr);
                ...
                {
                    MessageBoxExHelper.Instance.Show(MultiLanguageConvertHelper.Instance.Settings_Mappings_File_Is_Open_Please_Close_First, CommonInformations.Instance.GetFormMainTextByVersion(), MessageBoxButtons.OK, MessageBoxIcon.Warning); return;
                }

                TemplateHelper.BatchConsignDataTemplateHelper.Instance.LoadBOCDocument(fst, OperatorCommandType.CombineData, filepath);
            }
        }
        ...
        private void btnExport_Click(object sender, EventArgs e)
        {
            ...
            if (flag)
                ...
                MessageBoxExHelper.Instance.Show(MultiLanguageConvertHelper.Instance.DesignMain_Submit_Fail, CommonInformations.Instance.GetFormMainTextByVersion(), MessageBoxButtons.OK, MessageBoxIcon.Warning);
        }

        private void llOpenMapSetting_Click(object sender, EventArgs e)
        {
            CommandCenter.Instance.ResolveShowPanel(OperatorCommandType.View, AppliableFunctionType._Empty, FunctionInSettingType.MapSetting);
        }
        ...
        private void btnTypeConvert_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBoxExHelper.Instance.Show(string.Format("{0} {1}", MultiLanguageConvertHelper.Instance.Information_Please_Select, MultiLanguageConvertHelper.Instance.Settings_Convert_Data_Type), CommonInformations.Instance.GetFormMainTextByVersion(), MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
            if (string.IsNullOrEmpty(cmbDataFile.Text))
            {
                MessageBoxExHelper.Instance.Show(string.Format("{0} {1}", MultiLanguageConvertHelper.Instance.Information_Please_Select, MultiLanguageConvertHelper.Instance.Settings_Convert_Data_File), CommonInformations.Instance.GetFormMainTextByVersion(), MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
            ...
            if (!CommonInformations.Instance.CheckMappingSetting(aft, fst))
            {
                MessageBoxExHelper.Instance.Show(string.Format(MultiLanguageConvertHelper.Instance.Settings_Set_No_Mapping, cmbType.SelectedItem.ToString()), CommonInformations.Instance.GetFormMainTextByVersion(), MessageBoxButtons.OK, MessageBoxIcon.Warning); return;
            }
            else if (!(filepath.ToLower().EndsWith(".xls") || filepath.ToLower().EndsWith(".xlsx")))
            {
                if ((aft != AppliableFunctionType._Empty && string.IsNullOrEmpty(SystemSettings.Instance.BatchMappingSettings[aft].SeperateChar.Trim())) || (fst != FunctionInSettingType.Empty && string.IsNullOrEmpty(SystemSettings.Instance.BatchSettingsMappingSettings[fst].SeperateChar.Trim())))
                {
                    MessageBoxExHelper.Instance.Show(string.Format("{0}{1}", MultiLanguageConvertHelper.Instance.Information_Please_Input, "分隔符"), CommonInformations.Instance.GetFormMainTextByVersion(), MessageBoxButtons.OK, MessageBoxIcon.Warning); return;
                }
            }
            BatchHeader batch = null;
            MappingsRelationSettings mrs = new MappingsRelationSettings();
            if (aft != AppliableFunctionType._Empty)
                mrs = SystemSettings.Instance.BatchMappingSettings[aft];
            else if (fst != FunctionInSettingType.Empty)
                mrs = SystemSettings.Instance.BatchSettingsMappingSettings[fst];
                ...
                || aft == AppliableFunctionType.AgentExpressOut4Bar)
            {
                ...
                if ((aft == AppliableFunctionType.AgentNormalIn
                    ...
                    mrs.MaxCountPerOperation = SystemSettings.Instance.DefaultMaxCountAgentNormal;
            }
            ...
            //转换开始
            List<object> list = MatchFileDataHelper.Instance.GetFileData(aft, fst, filepath, mrs, batch);
            //转换结束
            frmWaiting.Close();
            if (list[0] is string && !string.IsNullOrEmpty(list[0] as string))
            {
                MessageBoxExHelper.Instance.Show(list[0].ToString(), CommonInformations.Instance.GetFormMainTextByVersion(), MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            else
            {
                    ...
                    && aft != AppliableFunctionType.TransferOverCountry4Bar)
                {
                    ...
                    {
                        sfDialog.Filter = string.Format("{0}|*.dif", MultiLanguageConvertHelper.Instance.DesignMain_FileType_DIF);
                        fileType = ".dif";
                    }
                    else
                    {
                        sfDialog.Filter = string.Format("{0}|*.txt", MultiLanguageConvertHelper.Instance.DesignMain_FileType_TXT);
                        fileType = ".txt";
                    }
                    if (sfDialog.ShowDialog() == System.Windows.Forms.DialogResult.OK)
                    {
                        filepath = DataConvertHelper.Instance.FormatFileName(sfDialog.FileName, fileType);
                        if (aft != AppliableFunctionType._Empty)
                            MatchFileDataHelper.Instance.SaveDataToTXT(list[1], aft, filepath);
                        else if (fst != FunctionInSettingType.Empty)
                            MatchFileDataHelper.Instance.SaveDataToTXT(list[1], fst, filepath);
                        MessageBoxExHelper.Instance.Show(MultiLanguageConvertHelper.Instance.Settings_Convert_Succeed, CommonInformations.Instance.GetFormMainTextByVersion(), MessageBoxButtons.OK, MessageBoxIcon.Information);
                    }
                }
                else
                {
                    ...
                    string filename = CommonInformations.Instance.GetBarFileName(aft, bankType);
                    ...
                    string filebar = DataConvertHelper.Instance.FormatFileName(filepath, ".bar");
                    bool result = MatchFileDataHelper.Instance.SaveDataToTXT(list[1], aft, filebar);
                    if (!result) { MessageBoxExHelper.Instance.Show("文件中无数据或者文件生成失败", CommonInformations.Instance.GetFormMainTextByVersion(), MessageBoxButtons.OK, MessageBoxIcon.Warning); return; }
                    ...
                    if (aft == AppliableFunctionType.AgentExpressOut4Bar)
                    {
                        ...
                        if (File.Exists(fpbarc1))
                            rd = FileDataPasswordBarHelper.Instance.EncodeAndZip(fpbarc1, fpc1, pwd);
                        if (File.Exists(fpbarc2))
                            rd = FileDataPasswordBarHelper.Instance.EncodeAndZip(fpbarc2, fpc2, pwd);
                    }
                    else
                        rd = FileDataPasswordBarHelper.Instance.EncodeAndZip(filebar, filepath, pwd);
        ...
        {
            OpenFileDialog ofDialog = new OpenFileDialog();
            ofDialog.Filter = string.Format("{0}|*.txt|{1}|*.csv|{2}|*.xls;*.xlsx", MultiLanguageConvertHelper.Instance.DesignMain_FileType_TXT, MultiLanguageConvertHelper.Instance.DesignMain_FileType_CSV, MultiLanguageConvertHelper.Instance.DesignMain_FileType_Excel);
        ...
        private void btnSelectAll_Click(object sender, EventArgs e)
        {
            ...
            CommandCenter.Instance.ResolveSettingsOperate(OperatorCommandType.SelectAll, fst);
        }
        ...
        private void btnSelectRe_Click(object sender, EventArgs e)
        {
            ...
            if ((sender as Controls.ThemedButton).Name.Equals(btnSelectRePayer.Name))
                ...
                fst = FunctionInSettingType.AgentExpressInPayerMg;
            CommandCenter.Instance.ResolveSettingsOperate(OperatorCommandType.SelectRe, fst);
        }
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            if ((sender as Controls.ThemedButton).Name.Equals(btnDeletePayer.Name))
                ...
                fst = FunctionInSettingType.AgentExpressInPayerMg;
            CommandCenter.Instance.ResolveSettingsOperate(OperatorCommandType.Delete, fst);
        }
