...
namespace BOC_BATCH_TOOL.VisualParts
{
    public partial class MenuControlPanel : BaseUc
    {
        ...
        void CommandCenter_OnShowPanelEventHandler(object sender, ShowPanelEventArgs e)
        {
            ...
            else
            {
                if (e.Command == EnumTypes.OperatorCommandType.View)
                {
                    ...
                    {
                        pictureBox1.BackgroundImage = global::BOC_BATCH_TOOL.Properties.Resources.settingselectedbg;
                    }
                    else
                    {
                        pictureBox1.BackgroundImage = global::BOC_BATCH_TOOL.Properties.Resources.settingred;
                    }
                }
            }
        }
            ...
            else
            {
                if (e.Command == EnumTypes.OperatorCommandType.MoveMenuCallBack)
                {
                    if (e.Endposition <= menuListPanel1.Width)
                    {
                        ...
                        pBack.BackgroundImage = global::BOC_BATCH_TOOL.Properties.Resources.grayright;
                    }
                    else if (e.Endposition > menuListPanel1.Width)
                    {
                        pCBack.Enabled = pBack.Enabled = true;
                        pBack.BackgroundImage = global::BOC_BATCH_TOOL.Properties.Resources.redright;
                    }

                    if (e.Startposition >= 0)
                    {
                        pCPre.Enabled = pPre.Enabled = false;
                        pPre.BackgroundImage = global::BOC_BATCH_TOOL.Properties.Resources.grayleft;
                    }
                    else if (e.Startposition < 0)
                    {
                        pCPre.Enabled = pPre.Enabled = true;
                        pPre.BackgroundImage = global::BOC_BATCH_TOOL.Properties.Resources.redleft;
                    }
        ...
        {
            CommandCenter.Instance.ResolveShowPanel(EnumTypes.OperatorCommandType.View, EnumTypes.AppliableFunctionType._Empty, EnumTypes.FunctionInSettingType.Empty);
        }
        ...
        private void pBack_Click(object sender, EventArgs e)
        {
            if (pBack.Enabled)
            {
                CommandCenter.Instance.ResolveMoveMenu(OperatorCommandType.MoveMenuRequest, 0, 0, 0, 0, 0, OperatorCommandType.MoveMenuReduce);
            }
        }
        ...
        private void pPre_Click(object sender, EventArgs e)
        {
            if (pPre.Enabled)
            {
                CommandCenter.Instance.ResolveMoveMenu(OperatorCommandType.MoveMenuRequest, 0, 0, menuListPanel1.Width, 0, 0, OperatorCommandType.MoveMenuRaise);
            }
        }
    }
}
