...
namespace CommonClient.MatchFile
{
    public class UpdateFileHelper
    {
        ...
        private static UpdateFileHelper m_instance;
        /// <summary>
        /// 单例
        /// </summary>
        public static UpdateFileHelper Instance
        {
            get
            {
                if (null == m_instance)
                {
                    lock (lock_obj)
                    {
                        ...
                        {
                            lock (lock_obj)
                                m_instance = new UpdateFileHelper();
                        }
                    }
                }
                return m_instance;
        ...
        internal SysCoach.ResultData Update(EnumTypes.UpdateFleType uft, string filepath)
        {
            SysCoach.ResultData rd = new SysCoach.ResultData { Result = true };
            List<string> fileList = new List<string>();
            fileList = ZipOperatorHelper.Instance.UnZip(filepath);
            if (!Directory.Exists("Data")) Directory.CreateDirectory(AppDomain.CurrentDomain.BaseDirectory + "\\Data");
            try
            {
                switch (uft)
                {
        ...
        void UpdateBankLinkCode(List<string> fileList, string targetfilename)
        {
        ...
        List<BankInfo> GetBankInfo(string filepath)
        {
            List<BankInfo> list = new List<BankInfo>();
            List<string> filedata = FileRWHelper.Instance.ReadCSVFieldsData(filepath);
            if (filedata.Count > 2)
            {
                filedata.RemoveAt(0);
                filedata.RemoveAt(0);
        ...
        void UpdateCNAPSCode(List<string> fileList, string targetfilename)
        {
        ...
        void UpdateClearBankCode(List<string> fileList, string targetfilename)
        {
            List<CNAP> list = new List<CNAP>();
        ...
        List<CNAP> GetCNAPS(string filepath)
        {
            List<CNAP> list = new List<CNAP>();
            List<string> filedata = FileRWHelper.Instance.ReadCSVFieldsData(filepath);
            if (filedata.Count > 2)
        ...
        void ReWriteDataFile(List<BankInfo> list, string filepath)
        {
        ...
        void ReWriteDataFile(List<CNAP> list, string filepath)
        {
            XmlDocument doc = new XmlDocument();
            XmlDeclaration declar = doc.CreateXmlDeclaration("1.0", "utf-8", "");
            doc.AppendChild(declar);
