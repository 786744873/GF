...
{
    public class R_Execute_reportingController : BaseController
    {

        private readonly ICommonService.Reporting.IR_Execute_reporting _reporting_CaseWCF;
        private readonly ICommonService.IC_Region _regionWCF;
        // GET: /Reporting/R_Execute_reporting/
        public R_Execute_reportingController()
        {
            _reporting_CaseWCF = ServiceProxyFactory.Create<ICommonService.Reporting.IR_Execute_reporting>("R_Execute_reportingWCF");
        ...
        public ActionResult AjaxStatistics(jQueryDataTableParamModel param)
        {
            ...
            //s_number
            //表单查询模型
            CommonService.Model.Reporting.R_Execute_reporting oFormConditon = new CommonService.Model.Reporting.R_Execute_reporting();
            ...
            {//代表已执行查询
                ...
                {
                    oFormConditon.R_Execute_reporting_area = Request.Params.Get("i_area");
                }

                if (!string.IsNullOrEmpty(Request.Params.Get("i_organ"))) //部门
                {
                    oFormConditon.R_Execute_reporting_organ = Request.Params.Get("i_organ");
                }

                if (!string.IsNullOrEmpty(Request.Params.Get("i_layyer"))) //主办
                {
                    oFormConditon.R_Execute_reporting_host = Request.Params.Get("i_layyer");
                }

                if (!string.IsNullOrEmpty(Request.Params.Get("i_number"))) //案件编码
                {
                    oFormConditon.R_Execute_reporting_caseNumber = Request.Params.Get("i_number");
                }

                if (!string.IsNullOrEmpty(Request.Params.Get("i_project"))) //工程
                {
                    oFormConditon.R_Execute_reporting_project = Request.Params.Get("i_project");
                }

                if (!string.IsNullOrEmpty(Request.Params.Get("i_court"))) //法院
                {
                    oFormConditon.R_Execute_reporting_firstCourt = Request.Params.Get("i_court");
                }

                if (!string.IsNullOrEmpty(Request.Params.Get("i_deleg"))) //委托人
                {
                    oFormConditon.R_Execute_reporting_plaintiff = Request.Params.Get("i_deleg");
                }

                if (!string.IsNullOrEmpty(Request.Params.Get("i_rival"))) //对方当事人
                {
                    oFormConditon.R_Execute_reporting_otherParty = Request.Params.Get("i_rival");
                }

                if (!string.IsNullOrEmpty(Request.Params.Get("i_yjbd"))) //移交标的
                {
                    oFormConditon.R_Execute_reporting_transferTarget = Request.Params.Get("i_yjbd");
                }

                if (!string.IsNullOrEmpty(Request.Params.Get("i_yjbdz"))) //移交标的至
                {
                    oFormConditon.R_Execute_reporting_transferTargetz = Request.Params.Get("i_yjbdz");
                }

                if (!string.IsNullOrEmpty(Request.Params.Get("i_yqsr"))) //预期收益
                {
                    oFormConditon.R_Execute_reporting_expectedReturn = Request.Params.Get("i_yqsr");
                }

                if (!string.IsNullOrEmpty(Request.Params.Get("i_yqsrz"))) //预期收入至
                {
                    oFormConditon.R_Execute_reporting_expectedReturnz = Request.Params.Get("i_yqsrz");
                }

                if (!string.IsNullOrEmpty(Request.Params.Get("i_time"))) //收案时间
                {
                    oFormConditon.R_Execute_reporting_closedDate = Request.Params.Get("i_time");
                }

                if (!string.IsNullOrEmpty(Request.Params.Get("i_timez"))) //收案时间至
                {
                    oFormConditon.R_Execute_reporting_closedDatez = Request.Params.Get("i_timez");
                }
            ...
            List<CommonService.Model.Reporting.V_Execute_reporting> listsLoan = _reporting_CaseWCF.GetDataList(oFormConditon, type, "", param.iDisplayStart + 1, param.iDisplayStart + param.iDisplayLength);
        ...
        public ActionResult AjaxList(jQueryDataTableParamModel param)
        {
            ...
            List<CommonService.Model.Reporting.R_Execute_reporting> listsLoan = _reporting_CaseWCF.GetListByPage(oFormConditon, " ID asc ", param.iDisplayStart + 1, param.iDisplayStart + param.iDisplayLength);

            //转化数据格式
            var result = from c in listsLoan
                         select new[] { 
                              c.R_Execute_reporting_area,
                              c.R_Execute_reporting_organName,   
                              c.R_Execute_reporting_host,
                              c.R_Execute_reporting_co,
                              c.R_Execute_reporting_firstCourt,
                              c.R_Execute_reporting_caseNumber,
                              c.R_Execute_reporting_plaintiff,
                              c.R_Execute_reporting_otherParty,
                              c.R_Execute_reporting_project,
                              c.R_Execute_reporting_closedDate,
                              c.R_Execute_reporting_transferTarget,
                              c.R_Execute_reporting_expectedReturn,
                              c.R_Execute_reporting_isExtension,
                              c.R_Execute_reporting_isExtension,
                              c.R_Execute_reporting_finishedTime,
        ...
        public FileResult ExportList()
        {
            ...
            List<CommonService.Model.Reporting.R_Execute_reporting> listsLoan = _reporting_CaseWCF.GetListByPage(oFormConditon, " ID asc ", 1, 1000000);
            ...
            for (int i = 0; i < listsLoan.Count; i++)
            {
                ...
                rowtemp.CreateCell(0).SetCellValue(listsLoan[i].R_Execute_reporting_area);
                rowtemp.CreateCell(1).SetCellValue(listsLoan[i].R_Execute_reporting_organName);
                rowtemp.CreateCell(2).SetCellValue(listsLoan[i].R_Execute_reporting_host);
                rowtemp.CreateCell(3).SetCellValue(listsLoan[i].R_Execute_reporting_co);
                rowtemp.CreateCell(4).SetCellValue(listsLoan[i].R_Execute_reporting_firstCourt);
                rowtemp.CreateCell(5).SetCellValue(listsLoan[i].R_Execute_reporting_caseNumber);
                rowtemp.CreateCell(6).SetCellValue(listsLoan[i].R_Execute_reporting_plaintiff);
                rowtemp.CreateCell(7).SetCellValue(listsLoan[i].R_Execute_reporting_otherParty);
                rowtemp.CreateCell(8).SetCellValue(listsLoan[i].R_Execute_reporting_project);
                rowtemp.CreateCell(9).SetCellValue(listsLoan[i].R_Execute_reporting_closedDate);
                rowtemp.CreateCell(10).SetCellValue(listsLoan[i].R_Execute_reporting_transferTarget);
                rowtemp.CreateCell(11).SetCellValue(listsLoan[i].R_Execute_reporting_expectedReturn);
                rowtemp.CreateCell(12).SetCellValue(listsLoan[i].R_Execute_reporting_isExtension);

                rowtemp.CreateCell(13).SetCellValue(listsLoan[i].R_Execute_reporting_isExtension);
                rowtemp.CreateCell(14).SetCellValue(listsLoan[i].R_Execute_reporting_finishedTime);
        ...
        /// </summary>
        /// <returns></returns>
        public FileResult ExportReporting()
        {
            #region 查询拼接条件设置,查询条件根据实际业务进行相应更改)
            //s_number
            //表单查询模型
            ...
            List<CommonService.Model.Reporting.V_Execute_reporting> listsLoan = _reporting_CaseWCF.GetDataList(oFormConditon, type, "", 1, 1000000);
