...
{
    public class R_Register_reportingController : BaseController
    {
        private readonly ICommonService.Reporting.IR_Register_reporting _reporting_CaseWCF;
        private readonly ICommonService.IC_Region _regionWCF;
        // GET: /Reporting/R_Register_reporting/
        public R_Register_reportingController()
        {
            _reporting_CaseWCF = ServiceProxyFactory.Create<ICommonService.Reporting.IR_Register_reporting>("R_Register_reportingWCF");
        ...
        public ActionResult AjaxStatistics(jQueryDataTableParamModel param)
        {
            ...
            //s_number
            //表单查询模型
            CommonService.Model.Reporting.V_Register_reporting oFormConditon = new CommonService.Model.Reporting.V_Register_reporting();
            ...
            {//代表已执行查询
                ...
                {
                    oFormConditon.R_Register_reporting_area = Request.Params.Get("i_area");
                }

                if (!string.IsNullOrEmpty(Request.Params.Get("i_organ"))) //部门
                {
                    oFormConditon.R_Register_reporting_organ = Request.Params.Get("i_organ");
                }

                if (!string.IsNullOrEmpty(Request.Params.Get("i_layyer"))) //主办
                {
                    oFormConditon.R_Register_reporting_host = Request.Params.Get("i_layyer");
                }

                if (!string.IsNullOrEmpty(Request.Params.Get("i_number"))) //案件编码
                {
                    oFormConditon.R_Register_reporting_caseNumber = Request.Params.Get("i_number");
                }

                if (!string.IsNullOrEmpty(Request.Params.Get("i_project"))) //工程
                {
                    oFormConditon.R_Register_reporting_project = Request.Params.Get("i_project");
                }

                if (!string.IsNullOrEmpty(Request.Params.Get("i_court"))) //法院
                {
                    oFormConditon.R_Register_reporting_firstCourt = Request.Params.Get("i_court");
                }

                if (!string.IsNullOrEmpty(Request.Params.Get("i_deleg"))) //委托人
                {
                    oFormConditon.R_Register_reporting_plaintiff = Request.Params.Get("i_deleg");
                }

                if (!string.IsNullOrEmpty(Request.Params.Get("i_rival"))) //对方当事人
                {
                    oFormConditon.R_Register_reporting_otherParty = Request.Params.Get("i_rival");
                }

                if (!string.IsNullOrEmpty(Request.Params.Get("i_yjbd"))) //移交标的
                {
                    oFormConditon.R_Register_reporting_transferTarget = Request.Params.Get("i_yjbd");
                }

                if (!string.IsNullOrEmpty(Request.Params.Get("i_yjbdz"))) //移交标的至
                {
                    oFormConditon.R_Register_reporting_transferTargetz = Request.Params.Get("i_yjbdz");
                }

                if (!string.IsNullOrEmpty(Request.Params.Get("i_yqsr"))) //预期收益
                {
                    oFormConditon.R_Register_reporting_expectedReturn = Request.Params.Get("i_yqsr");
                }

                if (!string.IsNullOrEmpty(Request.Params.Get("i_yqsrz"))) //预期收入至
                {
                    oFormConditon.R_Register_reporting_expectedReturnz = Request.Params.Get("i_yqsrz");
                }

                if (!string.IsNullOrEmpty(Request.Params.Get("i_time"))) //收案时间
                {
                    oFormConditon.R_Register_reporting_closedDate = Request.Params.Get("i_time");
                }

                if (!string.IsNullOrEmpty(Request.Params.Get("i_timez"))) //收案时间至
                {
                    oFormConditon.R_Register_reporting_closedDatez = Request.Params.Get("i_timez");
                }
            ...
            List<CommonService.Model.Reporting.V_Register_statistics> listsLoan = _reporting_CaseWCF.GetDataList(oFormConditon, type, "", param.iDisplayStart + 1, param.iDisplayStart + param.iDisplayLength);
        ...
        public ActionResult AjaxList(jQueryDataTableParamModel param)
        {
            ...
            List<CommonService.Model.Reporting.R_Register_reporting> listsLoan = _reporting_CaseWCF.GetListByPage(oFormConditon, " ID asc ", param.iDisplayStart + 1, param.iDisplayStart + param.iDisplayLength);

            //转化数据格式
            var result = from c in listsLoan
                         select new[] { 
                              c.R_Register_reporting_area,
                              c.R_Register_reporting_organName,   
                              c.R_Register_reporting_host,
                              c.R_Register_reporting_co,
                              c.R_Register_reporting_firstCourt,
                              c.R_Register_reporting_caseNumber,
                              c.R_Register_reporting_plaintiff,
                              c.R_Register_reporting_otherParty,
                              c.R_Register_reporting_project,
                              c.R_Register_reporting_closedDate,
                              c.R_Register_reporting_transferTarget,
                              c.R_Register_reporting_expectedReturn,
                              c.R_Register_reporting_isExtension,
                              c.R_Register_reporting_extensionTime,
                              c.R_Register_reporting_finishedTime,
        ...
        public FileResult ExportList(FormCollection form)
        {
            ...
            {//代表已执行查询
                ...
                {
                    oFormConditon.R_Register_reporting_area = form["i_area"];
                }

                if (!string.IsNullOrEmpty(form["i_organ"])) //部门
                {
                    oFormConditon.R_Register_reporting_organ = form["i_organ"];
                }

                if (!string.IsNullOrEmpty(form["i_layyer"])) //主办
                {
                    oFormConditon.R_Register_reporting_host = form["i_layyer"];
                }

                if (!string.IsNullOrEmpty(form["i_number"])) //案件编码
                {
                    oFormConditon.R_Register_reporting_caseNumber = form["i_number"];
                }

                if (!string.IsNullOrEmpty(form["i_project"])) //工程
                {
                    oFormConditon.R_Register_reporting_project = form["i_project"];
                }

                if (!string.IsNullOrEmpty(form["i_court"])) //法院
                {
                    oFormConditon.R_Register_reporting_firstCourt = form["i_court"];
                }

                if (!string.IsNullOrEmpty(form["i_deleg"])) //委托人
                {
                    oFormConditon.R_Register_reporting_plaintiff = form["i_deleg"];
                }

                if (!string.IsNullOrEmpty(form["i_rival"])) //对方当事人
                {
                    oFormConditon.R_Register_reporting_otherParty = form["i_rival"];
                }

                if (!string.IsNullOrEmpty(form["i_yjbd"])) //移交标的
                {
                    oFormConditon.R_Register_reporting_transferTarget = form["i_yjbd"];
                }

                if (!string.IsNullOrEmpty(form["i_yjbdz"])) //移交标的至
                {
                    oFormConditon.R_Register_reporting_transferTargetz = form["i_yjbdz"];
                }

                if (!string.IsNullOrEmpty(form["i_yqsr"])) //预期收益
                {
                    oFormConditon.R_Register_reporting_expectedReturn = form["i_yqsr"];
                }

                if (!string.IsNullOrEmpty(form["i_yqsrz"])) //预期收入至
                {
                    oFormConditon.R_Register_reporting_expectedReturnz = form["i_yqsrz"];
                }

                if (!string.IsNullOrEmpty(form["i_time"])) //收案时间
                {
                    oFormConditon.R_Register_reporting_closedDate = form["i_time"];
                }

                if (!string.IsNullOrEmpty(form["i_timez"])) //收案时间至
                {
                    oFormConditon.R_Register_reporting_closedDatez = form["i_timez"];
                }
                //string title = Request.Params.Get("s_number");
                //if (title != null && title != "")
                //{
            ...
            List<CommonService.Model.Reporting.R_Register_reporting> listsLoan = _reporting_CaseWCF.GetListByPage(oFormConditon, " ID asc ", 1, 1000000);
            ...
            for (int i = 0; i < listsLoan.Count; i++)
            {
                ...
                rowtemp.CreateCell(0).SetCellValue(listsLoan[i].R_Register_reporting_area);
                rowtemp.CreateCell(1).SetCellValue(listsLoan[i].R_Register_reporting_organName);
                rowtemp.CreateCell(2).SetCellValue(listsLoan[i].R_Register_reporting_host);
                rowtemp.CreateCell(3).SetCellValue(listsLoan[i].R_Register_reporting_co);
                rowtemp.CreateCell(4).SetCellValue(listsLoan[i].R_Register_reporting_firstCourt);
                rowtemp.CreateCell(5).SetCellValue(listsLoan[i].R_Register_reporting_caseNumber);
                rowtemp.CreateCell(6).SetCellValue(listsLoan[i].R_Register_reporting_plaintiff);
                rowtemp.CreateCell(7).SetCellValue(listsLoan[i].R_Register_reporting_otherParty);
                rowtemp.CreateCell(8).SetCellValue(listsLoan[i].R_Register_reporting_project);
                rowtemp.CreateCell(9).SetCellValue(listsLoan[i].R_Register_reporting_closedDate);
                rowtemp.CreateCell(10).SetCellValue(listsLoan[i].R_Register_reporting_transferTarget);
                rowtemp.CreateCell(11).SetCellValue(listsLoan[i].R_Register_reporting_expectedReturn);
                rowtemp.CreateCell(12).SetCellValue(listsLoan[i].R_Register_reporting_isExtension);

                rowtemp.CreateCell(13).SetCellValue(listsLoan[i].R_Register_reporting_extensionTime);
                rowtemp.CreateCell(14).SetCellValue(listsLoan[i].R_Register_reporting_finishedTime);
            ...
            string fileName = "立案报表" + dateTime + ".xls";
        ...
        public FileResult ExportReporting(FormCollection form)
        {
            ...
            List<CommonService.Model.Reporting.V_Register_statistics> listsLoan = _reporting_CaseWCF.GetDataList(oFormConditon, type, "", 1, 1000000);
