...
namespace Portal.Areas.FinanceManager.Controllers
{
    ...
    /// 费用借款单Control
    /// </summary>
    public class FeeLoanController : BaseController
    {
        ...
        public ActionResult List(FormCollection form, int? page = 1)
        {
            ...
            {//费用产生时间
                condition = condition + "and \"FBC53E44-D354-437E-9BDC-130EAE659C9F\" between '" + form["CostGenerationTime"] + "' and '" + form["CostGenerationTime1"] + "' ";
            ...
            {//发票种类
                condition = condition + "and \"B4C29E0B-524C-44A7-8FC6-5CCB91796003\" = " + form["InvoiceType"];
        ...
        /// 生成单据
        /// </summary>
        /// <param name="formPropertyValueGroup"></param>
        /// <returns></returns>
        public ActionResult GeneratingDocuments(string formPropertyValueGroup)
        {
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                CommonService.Model.C_Parameters parameter = InvoiceType.Where(p => p.C_Parameters_id == Convert.ToInt32(dr["B4C29E0B-524C-44A7-8FC6-5CCB91796003"])).FirstOrDefault();
                decimal InvoiceValue = Convert.ToDecimal(dr["90AF36AE-AC98-4503-8F76-B43FDDE07CD7"].ToString());//总金额
                if (!string.IsNullOrEmpty(dr["6A8926E0-58BC-45E1-ACD4-D88142275433"].ToString()))
                {
                    int caseCount = Convert.ToInt32(dr["6A8926E0-58BC-45E1-ACD4-D88142275433"].ToString());//案件
            ...
            //CommonService.Model.SysManager.C_Organization org = new CommonService.Model.SysManager.C_Organization();
            if (userinfo != null)
            {
                orgList = _organizationWCF.GetListByUserCode(applyPerson);
                    //organizations.Where(o => o.C_Organization_code == userinfo.C_Userinfo_Organization).FirstOrDefault();
            }
        ...
        public ActionResult Save(FormCollection form)
        {
            ...
            voucher.C_Voucher_documentType = Convert.ToInt32(DocumentTypeEnum.借款);
